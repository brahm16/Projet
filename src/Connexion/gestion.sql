...
--
-- Client :  127.0.0.1
-- GÃ©nÃ©rÃ© le :  Dim 09 FÃ©vrier 2020 Ã  17:30
...
CONSTRAINT `fk_entrepot` FOREIGN KEY (id_stocks)  REFERENCES `stocks` (`id_stocks`))
...
CONSTRAINT `fk_facture` FOREIGN KEY (id_stocks)  REFERENCES `stocks` (`id_stocks`))
) ENGINE=InnoDB DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;
...
CREATE TABLE IF NOT EXISTS `product_ordre` (
  `quantity` int(11) NOT NULL,
  `unity` varchar(255) NOT NULL,
  `id_product` int(11) NOT NULL,
  CONSTRAINT `fk_id` FOREIGN KEY (id_product)  REFERENCES `product` (`id_product`))
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CONSTRAINT `fk_stocks` FOREIGN KEY (id_product)  REFERENCES `product` (`id_product`))
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
